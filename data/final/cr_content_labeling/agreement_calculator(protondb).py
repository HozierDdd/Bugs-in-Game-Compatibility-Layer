"""Compute human-vs-LLM agreement on the ten CR content items for ProtonDB.

Compares the full manual labels (result/human_labeled/) against the LLM labels
(result/llm_labeled/) over all 384 sampled ProtonDB CRs, pooling the ten
items into binary (record x item) judgments.

Expected output:
Samples: 384
Label pairs: 3840
Cohen's Kappa: 0.8021
Raw Agreement: 0.9036
"""
...
def default_human_path() -> Path:
    return (
        ...
        / "result" / "human_labeled" / "protondb_label(manual_labeled).json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compute overall Cohen's Kappa between human and LLM labels for ProtonDB.",
    )
    parser.add_argument(
